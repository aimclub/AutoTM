...
if __name__ == "__main__":
    ...
    data_files = [(fpath, fpath_to_datetime(fpath)) for fpath in glob(f"{save_dir}/*.Rdata")]

    if len(data_files) == 0:
        logger.info(f"No data files have been found in {save_dir}. It is a normal situation. Interrupting execution.")
        sys.exit(0)

    last_data_filepath, _ = max(data_files, key=lambda x: x[1])
    recovery_filepath = os.path.join(save_dir, "recovery_rdata.checkpoint")

    logger.info(f"Copying last generated data file ({last_data_filepath}) as recovery file ({recovery_filepath})")
