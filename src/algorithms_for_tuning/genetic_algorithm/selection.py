...
def selection_fitness_prop(population, best_proc, children_num):
    ...
    c = 2
    for individ in population:
        updated_individ_fitness = np.max([(individ.fitness_value - (fitness_mean - c * fitness_std)), 0])
        cumsum_fitness += updated_individ_fitness
        individ._prob = updated_individ_fitness / cumsum_fitness
    if children_num == 2:
        return yield_matching_pairs(round((len(population) * (1 - best_proc)) // 2), population)
    else:
